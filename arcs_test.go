...
func TestBuildPreviousArcContext(t *testing.T) {
	...
	got := buildPreviousArcContext(state, 3, LangZH)
	...
	empty := buildPreviousArcContext(&Progress{}, 1, LangZH)
...
func TestOutlineConstraintsArcCompression(t *testing.T) {
	...
	got := buildOutlineConstraintsForLang(state, 3, LangZH) // writing chapter 4
	if !strings.Contains(got, "卷一摘要") {
