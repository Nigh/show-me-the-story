...
func GenerateArcSkeletonAction(ctx context.Context, apiCfg *APIConfig, cfg *Config, state *Progress, settings *ProjectSettings, progressPath, cfgPath string, logger *LogBroadcaster) error {
	...
	userPrompt := RenderPrompt(cfg.Prompts.ArcSkeleton, data)
...
func GenerateArcOutlineAction(ctx context.Context, apiCfg *APIConfig, cfg *Config, state *Progress, settings *ProjectSettings, arcID int, requirements, progressPath string, logger *LogBroadcaster) error {
	if err := validateAPIConfig(apiCfg); err != nil {
		return err
	}
...
func AppendArcAction(ctx context.Context, apiCfg *APIConfig, cfg *Config, state *Progress, settings *ProjectSettings, title, goal string, chapterCount int, progressPath string, logger *LogBroadcaster) error {
	if chapterCount <= 0 {
		chapterCount = 20
	}
	startCh := 1
...
func EnsureArcSummaries(ctx context.Context, apiCfg *APIConfig, cfg *Config, state *Progress, progressPath string, logger *LogBroadcaster) {
	for i := range state.Arcs {
		arc := &state.Arcs[i]
		...
		userPrompt := RenderPrompt(cfg.Prompts.ArcSummary, map[string]string{
			"Title":            preferUserValue(cfg.Story.Title, state.Title),
		...
		resp, err := CallAPI(ctx, apiCfg, SystemPromptFor(cfg.Language, "author_default"), userPrompt)
