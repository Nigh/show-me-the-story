...
只有末尾整批都未开始写作时，才显示「重新规划本批」，允许修改梗概与章数，确认后整体替换该批。失败时保留原批次和输入。旧项目的全书梗概保留兼容，但不会自动归入某一批次。标题栏显示已保存小说标题，未填写时显示“无题”。
...
技能页支持粘贴 Markdown、上传单个 `.md`、上传 ZIP 和浏览器选择文件夹四种安装方式。用户 Skill 安装到程序目录的全局 `skills/<id>/`，安装一次即可在不同小说项目中分别启用。标准包由 `skill.json`、`SKILL.md` 及可选的 `references/`、`templates/`、`assets/` 文本资源组成；脚本和二进制不会执行。旧项目目录中的 `skills/*.md` 仍可只读加载。
