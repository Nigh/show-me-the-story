...
<div class="rounded bg-base-200 p-3 space-y-2 text-sm">
  <h3 class="font-semibold">{$t('facts.title')}</h3>
  {#if pending.length}
    <div class="text-warning">{$t('facts.pending', {chapters: pending.join(', ')})}
  ...
  <div class="flex flex-wrap gap-1">
    {#each facts as f}
      <button class="btn btn-xs" class:btn-primary={activeFact?.id === f.id} on:click={() => activeFact = activeFact?.id === f.id ? null : f}>#{f.id} {f.content}</button>
    {/each}
  </div>
  {#if activeFact}
    ...
    <ul class="space-y-1 max-h-60 overflow-auto">
      {#each activeFact.references || [] as ref}
        <li>
          <button class="text-left w-full rounded p-1 hover:bg-base-300" disabled={ref.stale} on:click={() => dispatch('jump', ref)}>
