...
- Toast 的任务名称均通过 `task.*` 双语词条显示；文件上传与导出请求共用 `apiFetch`，携带当前 UI 语言并统一解析本地化错误，失败时不再误报下载成功。
- 首批大纲生成时若配置与进度中的小说标题均为空，模型会随批次章节返回推断标题并写入进度；后续批次或已有标题不会覆盖。
- 中文导航菜单使用“校订”作为完稿校订页的短标签，与其他导航项长度对齐；页面内仍使用完整名称。
- 完稿校订页的类型筛选保留英文枚举值用于 API 与存储，并通过 `proofread.category.*` 词条显示中英文标签。
- 自动校订全文完成后，校订页复用 `revisions[].changes[]` 显示修改章节／段落总数，并提供按章折叠的逐段校订前后对照；无修改时明确显示完成结果。
